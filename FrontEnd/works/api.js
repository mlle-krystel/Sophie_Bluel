...
export async function deleteProjet(projetID) {
  ...
  try {
    ...
    console.error("Impossible de supprimer ce projet :", error);
...
export async function ajoutPhotoApi(formData) {
  ...
  if (!token) {
    ...
    return false; //  Annule l'ajout si aucun token n'est trouvé
  }
  ...
  try {
    ...
    const response = await fetch("http://localhost:5678/api/works", {
      ...
      body: formData, // Envoi des données du formulaire sous forme de FormData
    });
    ...
    if (response.ok) {
      ...
      return false; // Annule si la requête a échoué
    }
  } catch (error) {
